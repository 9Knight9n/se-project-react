...
describe('facility-page-test', () => {
  ...
  test('facilities options render', () => {
    render(<BrowserRouter><Facilities /></BrowserRouter>);
    facilities.map(category => {expect(screen.getAllByText(category.label).length>0).toBe(true);})
  });
  ...
  facilities.map(facility => {
    test('facility '.concat(facility.label).concat(' css applied on selection') , () => {
      render(<BrowserRouter><Facilities /></BrowserRouter>);

      userEvent.click(screen.getAllByText(facility.label)[0])
      expect(screen.getByTestId('facility-select-test-'.concat(facility.id))).toHaveClass('selected');
