...
describe('photos page test', () => {
  ...
  // });
    ...
    await waitFor(() => {
      expect(container.querySelectorAll("div.ant-upload-list-item-done").length).toBe(1)
    })
    ...
    await waitFor(() => {
      expect(container.querySelectorAll("div.ant-upload-list-item-done").length).toBe(2)
  ...
  test('upload button removed after uploading 3 files for residence', async () => {
    ...
    for (let k=0;k<3;k++)
    {
      ...
      expect(screen.getByTestId('residence-doc-upload-button-add-villa')).toBeInTheDocument()
      await act(async () => {
        const input = screen.getByTestId('residence-doc-upload-button-add-villa')
        await fireEvent.change(input, {
          target: {
  ...
  test('upload button removed after uploading 3 files for user', async () => {
    ...
    for (let k=0;k<3;k++)
    {
      ...
      expect(screen.getByTestId('doc-upload-button-add-villa')).toBeInTheDocument()
      await act(async () => {
        const input = screen.getByTestId('doc-upload-button-add-villa')
        await fireEvent.change(input, {
          target: {
